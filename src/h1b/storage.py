...
def cache_root() -> str:
    ov = os.environ.get("AFL_H1B_CACHE_DIR")
    if ov:
        return ov
    r = _data_root()
    return join(r, "cache", "h1b", "cache") if is_remote(r) else join(r, "h1b-cache")


def output_root() -> str:
    ov = os.environ.get("AFL_H1B_OUTPUT_DIR")
    if ov:
        return ov
    r = _data_root()
    return join(r, "cache", "h1b", "output") if is_remote(r) else join(r, "h1b-output")
